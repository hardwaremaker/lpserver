...
im Anhang senden wir Ihnen unsere Anfrage <xsl:value-of select="/mail/bezeichnung"/> <xsl:value-of select="."/> zu Projekt <xsl:value-of select="/mail/xlsmailversand_projekt"/>
Ihr verbindliches Angebot erwarten wir bis zum <xsl:value-of select="/mail/xlsmailversand_abgabetermin"/>

Wir bitten Sie Ihre Preise in Stückzahlen in dem angehängten XLS einzutragen.
Wenn Sie bitte dieses XLS hinsichtlich Struktur und Datenaufbau wie übersandt zu belassen, damit wir dieses automatisiert verarbeiten können.
