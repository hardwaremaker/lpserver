...
<xsl:template match="belegnummer">hiermit erhalten Sie für Ihre Unterlagen die Lieferbestätigung zu Lieferschein <xsl:value-of select="."/>.

Die Lieferung erfolgte von <xsl:value-of select="/mail/bearbeiter_titel"/>&#160;<xsl:value-of select="/mail/bearbeiter_vorname"/>&#160;<xsl:value-of select="/mail/bearbeiter_nachname"/>.
