	...
	<xsl:template match="mail">
		...
		<html>
			<body>
				...
				<p>gerne &#252;bersenden wir Ihnen im Anhang
					<xsl:choose>
						<!-- grammatikalische Korrektur -->
						<xsl:when test="/mail/bezeichnung != 'der Lieferschein'">
							<xsl:value-of select="/mail/bezeichnung"/>
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="'den Lieferschein'"/>
						</xsl:otherwise>
					</xsl:choose>
					<xsl:choose>
				...
				<p>
					<xsl:choose>
						<xsl:when test="/mail/bearbeiter_email != ''">E-Mail:
							<xsl:value-of select="/mail/bearbeiter_email"/>
							<br/>
						</xsl:when>
					</xsl:choose>
					<xsl:choose>
						<xsl:when test="/mail/bearbeiter_telefonfirmamitdw != ''">Tel:
							<xsl:value-of select="/mail/bearbeiter_telefonfirmamitdw"/>
							<br/>
						</xsl:when>
					</xsl:choose>
					<xsl:choose>
						<xsl:when test="/mail/bearbeiter_mobil != ''">Mobil:
				...
				<p>
					<small>
						...
						| Geschäftsführer: <xsl:value-of select="/mail/mandant_geschaeftsfuehrer"/> | Ust.-ID:
						<xsl:value-of select="/mail/mandant_uid"/>
						<br/>
					</small>
				</p>
				<p>
					<small>Diese E-Mail wurde aus dem ERP-System HELIUM5 heraus erzeugt und ist vertraulich. Wenn Sie nicht der richtige Adressat sind oder
						<br/>
						diese E-Mail irrtümlich erhalten haben, informieren Sie bitte sofort den Absender und vernichten Sie dieses E-Mail.
						<br/>
						Das unerlaubte Kopieren sowie die unbefugte Weitergabe dieser E-Mails ist nicht gestattet.
						<br/>
						<br/>
						This e-Mail was created by the ERP System HELIUM5 and contains confidential and/or privileged information. If you are not the intended
						<br/>
